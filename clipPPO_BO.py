...
def load_prev_points(path):
    ...
    for di, d in enumerate(dirs):
        ...
        with open(os.path.join(d, next(x for x in fs if 'hyper' in x)), 'r') as f:
            p = json.load(f)
        p['h'], p['e'], p['l'] = p.pop('horizon'), p.pop('epsilon'), p.pop('lambda')
        p['mbs_p'], p['mbs_v'] = min(p['mbs_p'], p['h']), min(p['mbs_v'], p['h'])
        x[di] = [p['h'], p['e'], p['ep_p'], p['ep_v'], p['lr_p'], p['lr_v'], p['mbs_p'], p['mbs_v'], p['l']]

    return x, y


# function that saves a set of hyperparameter to disk
def write_hyper_params(path, h, e, ep_p, ep_v, lr_p, lr_v, mbs_p, mbs_v, l):
    params = {'horizon': h, 'epsilon': e, 'ep_p': ep_p, 'ep_v': ep_v,
...
def test(x):
    h, e, ep_p, ep_v, lr_p, lr_v, mbs_p, mbs_v, l = x[0]
    env_name = 'Swimmer-v1'
    path = '/home/polarbart/Documents/PPO_OAI_Test_discrete/' + env_name
    ...
    write_hyper_params(save_name, h, e, ep_p, ep_v, lr_p, lr_v, mbs_p, mbs_v, l)
    ...
    # learn(env, tf.Session(), 5000, .2, 20, 10, 3e-4, 3e-3, 256, 256, 1., .98)
    res = learn(env, tf.Session(), int(h), e, int(ep_p), int(ep_v), lr_p, lr_v, int(mbs_p), int(mbs_v), 1., l)
    print(res)
    return -res
...
while True:
    ...
    space = [{'type': 'discrete', 'domain': (256, 512, 1024, 2048, 4096, 8192)},  # h
             {'type': 'discrete', 'domain': (0.1, 0.2, 0.3)},  # e (0.1, 0.2, 0.3)
    ...
    ix, iy = load_prev_points('/home/polarbart/Documents/PPO_OAI_Test_discrete/Swimmer-v1')
